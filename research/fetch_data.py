...
def fetch_chunked_dvol():
    ...
    while cursor < NOW:
        ...
        resp = client._public(
            ...
            {"currency": "ETH", "start_timestamp": cursor, "end_timestamp": end, "resolution": 3600},
        )
...
with open(os.path.join(DATA_DIR, "candles_1h.json"), "w") as f:
    json.dump(candles, f)
with open(os.path.join(DATA_DIR, "funding_1h.json"), "w") as f:
    json.dump(funding, f)
with open(os.path.join(DATA_DIR, "dvol_1h.json"), "w") as f:
    json.dump(dvol, f)
